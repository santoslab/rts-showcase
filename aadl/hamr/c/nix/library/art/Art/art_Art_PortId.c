...
void art_Art_PortId_apply(Option_1CF021 result, String s) {
  char *endptr;
  errno = 0;
  unsigned long long n = strtoull(s->value, &endptr, 0);
  if (errno) {
    errno = 0;
    Type_assign(result, &((struct None_B2694B) { .type = TNone_B2694B }), sizeof(struct None_B2694B));
    return;
  }
  if (&s->value[s->size] - endptr == 0 && art_Art_PortId_C(0) <= n && n <= art_Art_PortId_C(91))
    Type_assign(result, &((struct Some_A0133F) { .type = TSome_A0133F, .value = (art_Art_PortId) n }), sizeof(struct Some_A0133F));
  else Type_assign(result, &((struct None_B2694B) { .type = TNone_B2694B }), sizeof(struct None_B2694B));
}