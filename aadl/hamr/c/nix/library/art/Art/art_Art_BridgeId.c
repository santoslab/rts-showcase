...
void art_Art_BridgeId_apply(Option_0E107D result, String s) {
  char *endptr;
  errno = 0;
  unsigned long long n = strtoull(s->value, &endptr, 0);
  if (errno) {
    errno = 0;
    Type_assign(result, &((struct None_D34309) { .type = TNone_D34309 }), sizeof(struct None_D34309));
    return;
  }
  if (&s->value[s->size] - endptr == 0 && art_Art_BridgeId_C(0) <= n && n <= art_Art_BridgeId_C(14))
    Type_assign(result, &((struct Some_9D1CD4) { .type = TSome_9D1CD4, .value = (art_Art_BridgeId) n }), sizeof(struct Some_9D1CD4));
  else Type_assign(result, &((struct None_D34309) { .type = TNone_D34309 }), sizeof(struct None_D34309));
}